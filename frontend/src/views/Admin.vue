...
<div  v-else>
    <div v-for="order in orders" :key="order.id" class="card mb-4 shadow-sm">
        <div class="card-header bg-primary text-white text-wrap text-break d-flex justify-content-between">
            <span>Order by:{{order.user?.name}}</span>|<span>({{order.user?.email}})</span>|
            <span>Total:{{order.totalPrice}}</span>
        </div>
        <div class="card-header bg-secondary text-white text-wrap text-break d-flex justify-content-between">
